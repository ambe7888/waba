(function (window, undefined) {
    ...
    window.__Utils = {
        ...
        *-------------------------------------------------------- */
        modalTemplatize: function (templateId, responseCallback, closeCallback, shownCallback) {
            ...
            if (shownCallback) {
                $(modalId).off('shown.bs.modal.lwTemplateModal').on('shown.bs.modal.lwTemplateModal', function (shownEvent) {
            ...
            if (closeCallback) {
                $(modalId).off((modalCloseEvent ? modalCloseEvent : 'hidden') + '.bs.modal.lwTemplateModal').on((modalCloseEvent ? modalCloseEvent : 'hidden') + '.bs.modal.lwTemplateModal', function (hiddenEvent) {
                    $(modalId).off('.lwTemplateModal');
    ...
    window.__DataRequest = {
        ...
        __protectedAjaxProcess: function (requestURL, inputData, responseCallback, requestMethod, options) {
            ...
            options = _.assign({
                csrf: (requestMethod != 'get') ? true : false,
                thisScope: $(this),
                preCallback: null,
                showMessage: false,
                    ...
                    if (errorResponse.status === 422) {
                        $.each(responseJSON.errors, function (key, value) {
                            ...
                            if ($thisScope.find('#' + underscoreKey + '-error').length) {
                                ...
                                var $thisFieldItem = $thisScope.find('[name="' + key + '"]');
                                if ($('.lw-error-container-' + $thisFieldItem.prop('name')).length) {
                                    $('.lw-error-container-' + $thisFieldItem.prop('name')).addClass('lw-validation-error').html(value).show();
                                } else if (($thisScope.find('.input-group-prepend ~ [name="' + key + '"]').length) || ($thisScope.find('[name="' + key + '"] ~ .input-group-append').length) || ($thisFieldItem.parents('.input-group').length)) {
                                    $('<div id="' + underscoreKey + '-error" class="lw-validation-error">' + value + '</div>').insertAfter($thisFieldItem.parents('.input-group')).show();
                                    ...
                                    $('<div id="' + underscoreKey + '-error" class="lw-validation-error">' + value + '</div>').insertAfter($($thisFieldItem[0])).show();
                                } else {
                                    $('<div id="' + underscoreKey + '-error" class="lw-validation-error">' + value + '</div>').insertAfter($thisFieldItem[0]).show();
                                }
                            }
    ...
    */
    ...
    /**
      ...
      *
      *-------------------------------------------------------- */
    $('body').off('click.ajaxFormSubmitAction').on('click.ajaxFormSubmitAction', '.lw-ajax-form-submit-action', function (e) {
        e.preventDefault();
        var $this = $(this),
