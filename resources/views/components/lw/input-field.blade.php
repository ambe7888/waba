{{-- Has a support for
    ...
    <div class="form-group mb-1 {{ $formGroupClass }}">
        ...
        @elseif(($dataType === 'selectize') or ($dataType === 'select'))
            @if ($dataType === 'select')
                <select {{ $dataType }} {!! $attributes->merge(['class' => 'lw-form-field form-control custom-select', 'id' => $id]) !!}>
                    {!! $selectOptions !!}
                </select>
            @elseif($dataType === 'selectize')
                <select data-lw-plugin="lwSelectize" {!! $attributes->merge(['class' => 'lw-form-field form-control custom-select', 'id' => $id]) !!}>
                    {!! $selectOptions !!}
                </select>
            @endif
        @else
