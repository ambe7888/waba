    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            <x-lw.datatable data-page-length="100" id="lwCampaignList" :url="route('vendor.campaign.read.list', ['status' => $status])">
                ...
                <th data-orderable="true" data-name="template_name">{{ __tr('Template/Preset Message') }}</th>
...
<% } else { %>
    ...
    <% if(__tData.current_status == 'PROCESSING') { %>
        <a data-method="post" href="<%= __Utils.apiURL("{{ route('vendor.campaign.write.abort', [ 'campaignIdOrUid']) }}", {'campaignIdOrUid': __tData._uid}) %>" class="btn btn-danger btn-sm lw-ajax-link-action-via-confirm" data-confirm="#lwCampaignAbort-template" title="{{ __tr('Abort') }}" data-callback-params="{{ json_encode(['datatableId' => '#lwCampaignList']) }}" data-callback="appFuncs.modelSuccessCallback"><i class="fas fa-ban"></i> {{  __tr('Abort') }}</a>
    <% } %>
    </script>
        <!-- /action template -->
        <!-- action template -->
