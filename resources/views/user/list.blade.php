            ...
            <x-lw.form id="lwAddNewUserForm" :action="route('vendor.user.write.create')"
                ...
                <div class="lw-form-modal-body">
                    ...
                    <fieldset>
                        ...
                        <div class="d-block my-3" x-data="{ isParentPermissionAllow: false }"">
                            ...
                            @if(!__isEmpty(data_get($permission, 'permissions')))
                                @foreach($permission['permissions'] as $subPermissionKey => $subPermission)
                                    <div class="ml-4">
                                        ...
                                        <p class="text-muted mt-1 fs-1">{{ $subPermission['description'] }}</p>
                                        @endif
                                    </div>
                                @endforeach
                            @endif
        ...
        <x-lw.modal id="lwEditUser" :header="__tr('Edit User & Permissions')" :hasForm="true">
           ...
           <x-lw.input-field type="text" id="lwLastNameEditField" data-form-group-class="" :label="__tr('Last Name')" value="<%- __tData.last_name %>" name="last_name"  required="true"                 />
            ...
            @if(hasVendorAccess('hide_contact_emails') and getUserAuthInfo('role_id') !== 2)
                <x-lw.input-field type="text" id="lwMobileNumberEditField" data-form-group-class="" :label="__tr('Mobile Number')" value="<%- __tData.mobile_number %>" disabled name="mask_mobile_number"  />
            @else
            ...
            @if(hasVendorAccess('hide_contact_emails') and getUserAuthInfo('role_id') !== 2)
                <x-lw.input-field type="text" disabled id="lwEmailEditField" data-form-group-class="" :label="__tr('Email')" value="<%- __tData.email %>" name="mask_email"  />
            @else
           ...
           <x-lw.input-field type="password" id="lwPasswordEditField" data-form-group-class="" :label="__tr('Password')"  name="password"  />
                ...
                <fieldset>
                    ...
                    <% var mainPermission = __tData.vendor_user_details?.__data?.permissions?.{{ $permissionKey }}; %>
                        <span class="d-block my-3" x-data="{ isParentPermissionAllow: <%- __tData.vendor_user_details?.__data?.permissions?.{{ $permissionKey }} == 'allow' ? true : false %> }">
                            <label for="lwEdit{{ $permissionKey }}Permission" class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-600 lw-switchery-label">{{ $permission['title'] }}</span>
                            </label>
                            @if (isset($permission['description']) and $permission['description'])
                            <p class="text-muted mt-1 fs-1">{{ $permission['description'] }}</p>
                            @endif
                            @if(!__isEmpty(data_get($permission, 'permissions')))
                                @foreach($permission['permissions'] as $subPermissionKey => $subPermission)
                                    ...
                                    %>
                                    <span class="ml-4">
                                    <label for="lwEdit{{ $subPermissionKey }}SubPermission" class="flex items-center">
                                        ...
                                        <span class="ml-1 text-gray-600 lw-switchery-label">{{ $subPermission['title'] }}</span>
                                    </label>
                                    <span>
                                    @if (isset($subPermission['description']) and $subPermission['description'])
                                        <p class="text-muted mt-1 fs-1">{{ $subPermission['description'] }}</p>
...
<!-- /login as button -->
        ...
        <script type="text/template" id="lwDeleteUser-template">
            ...
            <p>{{ __tr('You want to delete this team member?') }}</p>
    </script>
        <!-- /User delete template -->
        <script type="text/template" id="lwLoginAs-template">
            <h2>{{ __tr('Are You Sure!') }}</h2>
        <p>{{ __tr('You want login to this team member account?') }}</p>
</script>
    </div>
</div>
@endsection()