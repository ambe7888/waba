...
/**
 * Core Helper - 1.25.53 - 08 APR 2026
 *
 ...
 *-------------------------------------------------------- */
// generate unique request id and store in config for current request life cycle
config([
    'app.__unique_request_id' => uniqid(),
]);
    ...
    function __clog()
    {
        ...
        return config([
            'app.__clog.' . count(config('app.__clog', [])) => array_map(function ($argument) {
                return print_r('Unique Request ID: ' . config('app.__unique_request_id'), true) . "\n\r" . print_r($argument, true);
            }, $args),
        ]);
    }
}
...
if ((config('app.debug', false) == true)
    ...
    // prevent multiple registration of listener
    if (! app()->bound('db_listener_registered')) {
        app()->instance('db_listener_registered', true);
        if (version_compare(Illuminate\Foundation\Application::VERSION, '10.0.0', '>')) {
            // Laravel version is greater than 10
            Illuminate\Support\Facades\DB::listen(function (Illuminate\Database\Events\QueryExecuted $query) {
                ...
                $querySql = vsprintf($querySql, $bindings);
                $clogItems = ['SQL__Query' => $querySql];
                $clogItems['SQL__TimeTaken'] = $query->time;
                foreach (debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS) as $trace) {
                    if (isset($trace['file']) && Str::contains($trace['file'], base_path('app'))) {
                        $clogItems['SQL__RequestLocation'] = $trace['file'] . ':' . $trace['line'];
                        __clog($clogItems);
                        __logDebug($clogItems);
                        break;
                    }
                }
            });
        } else {
            // Laravel version is less than 10
...
if (! function_exists('viaFlashCache')) {
    function viaFlashCache(string $cacheKey, $originalData = null, $options = [])
    {
        $options = array_merge([
            'force_fresh' => false
        ], $options);
        if ($options['force_fresh'] === true) {
            emptyFlashCache($cacheKey);
        }
        // fetch cached app settings from config
        $data = flashCacheStore($cacheKey, 'FLASH_VALUE_NOT_FOUND');
        if ($data === 'FLASH_VALUE_NOT_FOUND') {
        ...
        // $data = flashCacheStore($cacheKey, null);
        return $data;
    }
}
 ...
 * @modified  1.25.53 - 08 APR 2026
 *---------------------------------------------------------------- */
if (! function_exists('emptyFlashCache')) {
    function emptyFlashCache(string $cacheKey = '')
    {
        // if cache key is not sent or empty string reset the whole flash cache store
        if (!$cacheKey or ($cacheKey === '')) {
            // reset the whole flash cache store if cache key is not sent or empty string
            flashCacheStore('RESET_FLASH_CACHE_STORE');
        }
        // store blank cache in config as in memory
        flashCacheStore([
            $cacheKey => 'FLASH_VALUE_NOT_FOUND',
        ]);
...
if (! function_exists('flashCacheStore')) {
    /**
     * Key to store
     *
     ...
     * @return mixed
     */
    function flashCacheStore($key, $default = null)
    {
        // Ensure request-level store exists
        if (! app()->bound('__FLASH_CACHE_STORE__')) {
            app()->instance('__FLASH_CACHE_STORE__', []);
        }
        $store = app('__FLASH_CACHE_STORE__');
        // SETTER (array input)
        if ($key === 'RESET_FLASH_CACHE_STORE') {
            app()->instance('__FLASH_CACHE_STORE__', []);
            return true;
        }
        if (is_array($key)) {
            $store = array_replace_recursive($store, $key);
            app()->instance('__FLASH_CACHE_STORE__', $store);
            return true;
        }
        // Getter with default
        return Arr::get($store, $key, $default);
    }
}
