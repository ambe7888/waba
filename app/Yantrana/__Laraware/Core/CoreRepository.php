...
/**
 * Core Repository - 1.12.13 - 09 JAN 2026
 *
...
abstract class CoreRepository
{
     ...
     *
     * @last-modified 1.12.13 - 09 JAN 2026
     *---------------------------------------------------------------- */
    public function fetchIt($idOrUid, $options = [])
    {
        ...
        // generate cache key
        $flashDbRequestCacheKey = $this->dbRequestFlashCacheKey([$idOrUid, $options]);
        // return the result
        return viaFlashCache('fetchIt_' . $flashDbRequestCacheKey, function () use ($idOrUid) {
    ...
    public function fetchItAll($idOrUid = null, $columns = [], $whereInKey = null, $options = [])
    {
        ...
        // generate cache key
        $flashDbRequestCacheKey = $this->dbRequestFlashCacheKey([$idOrUid, $whereInKey, $options, $columns]);
        // return the result
        return viaFlashCache('fetchItAll_' . $flashDbRequestCacheKey, function () use ($idOrUid, $whereInKey, $options, $columns) {
    ...
    public function countIt($idOrUid = null, $whereInKey = null, $options = [])
    {
        ...
        // generate cache key
        $flashDbRequestCacheKey = $this->dbRequestFlashCacheKey([$idOrUid, $whereInKey, $options]);
        return viaFlashCache('countIt_' . $flashDbRequestCacheKey, function () use ($idOrUid, $whereInKey, $options) {
    ...
    public function updateIt($eloquentModel, $inputData)
    {
        ...
        if ($eloquentModel and ($eloquentModel->modelUpdate($inputData))) {
            return true;
        }
    ...
    /**
     * Generate flash cache key
     *
     * @param  array  $array
     * @return string
     *---------------------------------------------------------------- */
    protected function dbRequestFlashCacheKey($array)
    {
        return sha1(json_encode([class_basename(static::class), $array]));
    }
}
