...
class WhatsAppServiceEngine extends BaseEngine implements WhatsAppServiceEngineInterface
{
    ...
    public function campaignRequiredData()
    {
        ...
        return $this->engineSuccessResponse([
            ...
            'vendorAudiences' => $vendorAudiences,
            'template' => '',
            'templatePreview' => '',
            'allLabels' => $allLabels,
    ...
    {
        $audienceUid = data_get($inputData, 'audience_uid');
        $contactGroupIds = data_get($inputData, 'group_contact_ids', []);
        $vendorId = getVendorId();

        if (__isEmpty($inputData['template_id']) or (empty($audienceUid) && empty($contactGroupIds))) {
            return $this->engineSuccessResponse([
                'totalContacts' => 0,
            ]);
        }
    ...
    public function processCampaignCreate($request)
    {
        ...
        $audienceUid = $request->audience_uid;
        ...
        if ($presetMessageUid && empty($audienceUid) && empty($request->contact_group)) {
            ...
            // if not all contacts
            if (!empty($contactGroupIds) && !in_array('all_contacts', $contactGroupIds)) {
                $contactGroup = $this->contactGroupRepository->fetchItAll($contactGroupIds, [], '_id', [
                    'where' => [
                        'vendors__id' => $vendorId
                    ]
    ...
    {
        // 'api_access' is a different feature (it gates apiSendChatMessage()
        // etc. in WhatsAppServiceController - the vendor's own developer
        // API/webhooks for third-party integrations), not "may this vendor
        // connect a real WhatsApp Business account at all". Connecting an
        // account is the base product, so this uses the same
        // hasActivePlan() check as sendTemplateMessageProcess() and the
        // other message-sending gates in this file.
