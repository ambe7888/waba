                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final String templateName =
                          campaign['template_name'] ?? 'Modèle inconnu';
                                  ...
                                  children: [
                                    TextButton.icon(
                                      icon: const Icon(Icons.bar_chart_rounded,
                                          size: 16),
                                      label: const Text('Voir le dashboard',
                                          style: TextStyle(fontSize: 12)),
                                      onPressed: () =>
                                          _openCampaignDashboard(campaign),
                                    ),
                                  ],
                                ),
                              ],
...
class _CampaignDashboardScreenState extends State<CampaignDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final String status =
        _stats?['campaignStatus'] ?? widget.campaign['status'] ?? '...';
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      GridView.count(
                        ...
                        children: [
                          _statCard('Envoyés', _stats!['totalSent'],
                              Colors.blue, Icons.send_rounded),
                          _statCard('Délivrés', _stats!['totalDelivered'],
                              Colors.green, Icons.done_all_rounded),
                          _statCard('Lus', _stats!['totalRead'], Colors.purple,
                              Icons.visibility_rounded),
                          _statCard('Échoués', _stats!['totalFailed'],
                              Colors.red, Icons.error_outline_rounded),
                          _statCard('En attente', _stats!['inQueueCount'],
                              Colors.orange, Icons.hourglass_empty_rounded),
                          _statCard('Expirés', _stats!['expiredCount'],
                              Colors.grey, Icons.timer_off_rounded),
                        ],
                      ),
                      const SizedBox(height: 16),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            _percentRow('Taux de livraison',
                stats['totalDeliveredInPercent'] ?? '0%', Colors.green),
            _percentRow('Taux de lecture', stats['totalReadInPercent'] ?? '0%',
                Colors.purple),
            _percentRow('Taux d\'envoi', stats['totalSentInPercent'] ?? '0%',
                Colors.blue),
            _percentRow('Taux d\'échec', stats['totalFailedInPercent'] ?? '0%',
                Colors.red),
          ],
        ),
      ),
  ...
  Future<void> _submitCampaign() async {
    ...
    if (_selectedGroupId == null || _selectedTemplate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Veuillez choisir un groupe et un modèle.')),
      );
      return;
    }
    ...
    _variableControllers.forEach((key, c) {
      payload[key] = c.text.trim();
    });
                ...
                child: ListView(
                  children: [
                    ...
                    if (_bodyVariables.isNotEmpty) ...[
                      _sectionLabel('Variables du modèle',
                          color: Colors.indigo),
                      const SizedBox(height: 8),
